...
// --- UI Elementen ---
// Deze worden gevuld zodra de DOM geladen is.
...
async function connectToFirebase() {
    try {
        ...
        if (error.code !== 'auth/custom-token-mismatch') { // Voorkom dubbele foutmeldingen
    ...
    onSnapshot(q, (snapshot) => {
        ...
        opgeslagenKlussenLijst.innerHTML = docs.map(doc => {
            try {
                ...
                return `<div class="bg-white p-3 rounded-lg border border-slate-200 shadow-sm list-item-enter"><p class="font-semibold text-slate-800">${plan.klusTitel}</p><p class="text-sm text-slate-500">${plan.schattingWerkdagen.min}-${plan.schattingWerkdagen.max} dagen</p></div>`;
            } catch (e) { return ''; }
        }).join('');
    }, (error) => {
